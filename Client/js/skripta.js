...
var pretragaEndpoint = "api/ribe/pretraga";
            ...
            .then((response) => {
                if (response.status === 200) {
                    ...
                    response.json().then(function(data) {
                        console.log(data);
                        document.getElementById("info").innerHTML = "<br>Prijavljeni korisnik: <b>" + data.username + "<b/>.";
...
function loadRibarnice() {
    var ribarniceUrl = host + port + "api/ribarnice/"; 
...
function createHeader() {
    ...
    if(jwt_token){
            row.appendChild(createTableCellTh("Sorta"));
            row.appendChild(createTableCellTh("Cena (din/kg)"));
            row.appendChild(createTableCellTh("Ribarnica"));
            row.appendChild(createTableCellTh("Kolicina (kg)"));
            row.appendChild(createTableCellTh("Mesto ulova"));
            row.appendChild(createTableCellTh("Akcija"));
        } else {
            row.appendChild(createTableCellTh("Sorta"));
            row.appendChild(createTableCellTh("Cena (din/kg)"));
            row.appendChild(createTableCellTh("Ribarnica"));
...
function setRiba(data) {
    ...
    var headingText = document.createTextNode("Riba na prodaju");
    ...
    for(var i = 0; i < data.length; i++)
    {
        ...
        if(jwt_token){
            ...
            row.appendChild(createTableCell(data[i].mestoUlova));
...
function submitRibaForm(){
    ...
    var url;
        ...
        sendData = {
            "Sorta": ribaSorta,
            "MestoUlova": ribaMestoUlova,
            "Cena": ribaCena,
            "DostupnaKolicina": ribaUkupnaKolicina,
            "RibarnicaId": ribaRibarnicaId
        };
...
function submitPretragaRibePoUkupnojKolicini() {
	...
	var sendData = {
			"Najmanje": pretragaStart,
			"Najvise": pretragaKraj,
		};
